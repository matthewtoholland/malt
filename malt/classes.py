...
class Molecule:
    ...
    def get_esp_cube_lines(self):
        ...
        min_carts = ang2a0 * (min_cat_values - 5 * np.ones(3))
        max_carts = ang2a0 * (max_cart_values + 5 * np.ones(3))

        coords = np.array([ang2a0 * np.array(coord) for coord in coords])
        ...
        cube_file_lines = ['Generated by malt\n', 'ESP\n']
        ...
        for atom in self.atoms:
            ...
            charge = atom[5]
            symbol = atom[0][0]
            atomic_no = Chem.GetPeriodicTable().GetAtomicNumber(symbol)
            cube_file_lines.append(f'{atomic_no:>5d}{charge:>12f}'
                                f'{ang2a0*x:>12f}{ang2a0*y:>12f}{ang2a0*z:>12f}\n')
